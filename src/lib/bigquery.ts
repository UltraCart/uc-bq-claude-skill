...
const STANDARD_TABLES: TableInfo[] = [
  ...
  { table_name: 'uc_integration_logs', description: 'One row per interaction with a third-party integration (payment gateway, shipping carrier, tax service, ESP/marketing platform, webhook, channel partner, distribution center, etc.) identified by log_type (~100 types). Fields include status, action, direction, associated order_ids / auto_order_oids / email / item, primary log text, structured log_map_entries and attributes (key/value pairs), and attached files with JSON/text/binary content inline when small. Low/standard tier strips email, log text, and file content; medium/high retain all PII.' },
